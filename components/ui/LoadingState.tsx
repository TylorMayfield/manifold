...
const LoadingState: React.FC<LoadingStateProps> = ({
    ...
    return (
      ...
      >
        <div className="max-w-2xl mx-auto p-8 text-center">
          <div className="mb-8">
            <div className="mx-auto w-32 h-32 rounded-full bg-tangerine-500/20 flex items-center justify-center mb-6">
              <Loader2 className="w-16 h-16 text-tangerine-400 animate-spin" />
            </div>
            <h2 className="text-2xl font-bold text-white mb-2">{message}</h2>
            {submessage && (
              <p className="text-dark_cyan-400 text-lg">{submessage}</p>
            )}
          </div>
        </div>
      </div>
    ...
    return (
      <div className={cn("p-12 text-center", className)}>
        <Loader2
          className={cn(
            spinnerSizes[size],
            "mx-auto mb-4 animate-spin text-white/50"
          )}
        />
  ...
  if (variant === "spinner") {
    return (
      <Loader2
        className={cn(sizeClasses[size], "animate-spin text-white", className)}
      />
    );
  }
