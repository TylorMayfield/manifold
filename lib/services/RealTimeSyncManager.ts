...
export class RealTimeSyncManager extends EventEmitter {
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      logger.error("Failed to start real-time sync manager", {
        category: "realtime-sync",
        error: error.message,
      });
      throw error;
  ...
  removeDataSource(sourceId: string): void {
    ...
    logger.info("Data source removed", {
      category: "realtime-sync",
      sourceId,
    });
  ...
  private setupWebSocketHandlers(): void {
    this.wsService.on("connected", () => {
      logger.info("WebSocket connected for real-time sync", {
        category: "realtime-sync",
      });
      this.emit("connected");
    });
    ...
    this.wsService.on("disconnected", () => {
      logger.warn("WebSocket disconnected for real-time sync", {
        category: "realtime-sync",
      });
    ...
    this.wsService.on("error", (error: Error) => {
      logger.error("WebSocket error in real-time sync", {
        category: "realtime-sync",
        error: error.message,
      });
  ...
  private startSourceSync(source: DataSource): void {
    ...
    logger.info("Started sync for data source", {
      category: "realtime-sync",
      sourceId: source.id,
      interval,
    });
