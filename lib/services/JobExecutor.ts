...
import { ScheduledTask, TaskHistory } from "./SystemMonitor";
...
export class JobExecutor {
  ...
  async executeTask(task: ScheduledTask): Promise<boolean> {
    const context: JobExecutionContext = {
      ...
      onComplete: (success, error, result) => {
        logger.info(
          `Task ${task.id} completed: ${success}`,
          "data-processing",
          { error, result }
        );
      },
    ...
    try {
      logger.info(`Starting task execution: ${task.name}`, "data-processing", {
        taskId: task.id,
      });
      ...
      logger.error(`Task execution failed: ${task.name}`, "data-processing", {
        error,
        taskId: task.id,
      });
      this.runningJobs.delete(task.id);
  ...
  private async executeBackupTask(
    ...
    try {
      ...
      logger.success(`Backup task completed: ${task.name}`, "backup", {
        taskId: task.id,
        projectId: task.projectId,
      ...
      logger.error(`Backup task failed: ${task.name}`, "backup", {
        error,
        taskId: task.id,
      });
      return false;
  ...
  private async executeCustomScriptTask(
    ...
    try {
      ...
      const dataSource = await this.dbService.getDataSource(task.dataSourceId);
      if (!dataSource || dataSource.type !== "custom_script") {
        throw new Error(
      ...
      logger.success(
        `Custom script task completed: ${task.name}`,
        "data-processing",
        {
          taskId: task.id,
          dataSourceId: task.dataSourceId,
      ...
      logger.error(
        `Custom script task failed: ${task.name}`,
        "data-processing",
  ...
  async cancelJob(taskId: string): Promise<boolean> {
    if (this.runningJobs.has(taskId)) {
      ...
      logger.info(`Job cancelled: ${taskId}`, "data-processing");
      return true;
    }
    return false;
  }
  ...
  async createJobsFromDataSources(projectId: string): Promise<ScheduledTask[]> {
    ...
    for (const dataSource of dataSources) {
      ...
      if (dataSource.type === "mysql" || dataSource.type === "api") {
  ...
  async createBackupJobs(projectId: string): Promise<ScheduledTask[]> {
    ...
    logger.info(`Created backup job for project: ${projectId}`, "backup");
    return [backupJob];
  }
}
