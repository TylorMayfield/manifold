...
export class DataQualityMonitor {
  ...
  createRule(
    ...
    logger.info("Quality rule created", "system", {
      ruleId: id,
      ruleName: rule.name,
      ruleType: rule.type,
    });
  ...
  updateRule(id: string, updates: Partial<QualityRule>): QualityRule | null {
    ...
    logger.info("Quality rule updated", "system", {
      ruleId: id,
      updates,
    });
  ...
  deleteRule(id: string): boolean {
    ...
    if (deleted) {
      logger.info("Quality rule deleted", "system", {
        ruleId: id,
      });
    }
  ...
  async checkDataQuality(
    ...
    logger.info("Starting data quality check", "system", {
      dataset,
    ...
    logger.info("Data quality check completed", "system", {
      dataset,
      overallScore,
      processingTime,
  ...
  private async executeRule(
    ...
    try {
      ...
      logger.error("Rule execution failed", "system", {
        ruleId: rule.id,
  ...
  private async checkCustom(
    ...
    try {
      ...
      logger.error("Custom rule execution failed", "system", {
        ruleId: rule.id,
        error: error instanceof Error ? error.message : String(error),
      });
  ...
  acknowledgeAlert(alertId: string): boolean {
    for (const alerts of this.alerts.values()) {
      ...
      if (alert) {
        ...
        logger.info("Alert acknowledged", "system", {
          alertId,
        });
        return true;
      }
  ...
  resolveAlert(alertId: string): boolean {
    for (const alerts of this.alerts.values()) {
      ...
      if (alert) {
        alert.resolved = true;
        logger.info("Alert resolved", "system", {
          alertId,
        });
        return true;
      }
